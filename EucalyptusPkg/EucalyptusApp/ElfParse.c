  ...
  )
{
  ...
  if (EFI_ERROR(Status)) {
    Print(L"ERROR: Failed to open payload.elf: %r\n", Status);
    Root->Close(Root);
    return Status;
  }

  Print(L"-> payload.elf found!\n");
  ...
  Print(L"-> payload buffer at 0x%p\n", PayloadBuffer);
  ...
  Print(L"-> executing payload at 0x%lx, good night\n", Entry);

  VOID (*Jump)(VOID) = (VOID (*)(VOID))(UINTN)Entry;
  PrepareHandoff();
  Jump();

  Print(L"-> payload returned\n");
  while (TRUE);
